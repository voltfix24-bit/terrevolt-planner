...
-- - Monteur-register met privacy-afscherming
-- - Dagregels/correcties voor uren
-- - Export-RPC uit bestaande planning
-- - Exportlog voor later bewijs/factuurcontrole
-- =====================================================================

-- Dienstverband is apart van vakgebied/type.
...
-- PII en ZZP-gegevens bewust los van public.monteurs.
...
-- Privacy-invariant: ZZP krijgt nooit BSN opgeslagen.
...
AS $$
DECLARE
  v_dienstverband text;
BEGIN
  SELECT dienstverband INTO v_dienstverband
  FROM public.monteurs
  WHERE id = NEW.monteur_id;
...
-- Wissel naar ZZP wist bestaand BSN automatisch.
...
BEGIN
  IF NEW.dienstverband = 'zzp' AND OLD.dienstverband IS DISTINCT FROM 'zzp' THEN
    UPDATE public.monteur_register
    SET bsn = NULL
    WHERE monteur_id = NEW.id;
  END IF;
  RETURN NEW;
END;
$$;
...
-- Correctie-/controlelaag: planning blijft bron, deze tabel bewaart afwijkende uren/status/notities.
...
-- Exportlog: bewijs dat een uitdraai is gemaakt.
...
-- RLS: PII/register en correcties alleen manager.
...
-- Helper voor ISO datum uit jaar/week/dag_index. dag_index 0 = maandag.
...
-- Export: 1 rij per project/monteur/dag, geaggregeerd uit bestaande planning.
...
RETURNS TABLE (
  ...
  status text,
  activiteiten text,
  activiteit_count integer,
  compleet boolean,
  ontbrekende_velden text[]
)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  WITH geplande_dagen AS (
    ...
    LEFT JOIN public.mandagen_regels mr
      ON mr.project_id = gd.project_id
     AND mr.monteur_id = gd.monteur_id
     AND mr.datum = gd.datum
  ...
  SELECT
    ...
    v.activiteiten,
    v.activiteit_count,
    cardinality(v.ontbrekende_velden) = 0 AS compleet,
    v.ontbrekende_velden
  FROM verrijkt v
  ORDER BY v.dienstverband, v.naam, v.datum;
END;
$$;

-- Manager kan uren/status per geplande dag corrigeren.
CREATE OR REPLACE FUNCTION public.upsert_mandagen_regel(
  p_project_id uuid,
  p_monteur_id uuid,
  p_datum date,
  p_uren numeric,
  p_status text DEFAULT 'gecontroleerd',
  p_opmerking text DEFAULT NULL
)
RETURNS public.mandagen_regels
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_row public.mandagen_regels;
BEGIN
  IF NOT public.is_planner_manager(auth.uid()) THEN
    RAISE EXCEPTION 'Geen toegang tot mandagenregister.' USING ERRCODE = '42501';
  END IF;
...
-- Log een export en markeer bestaande correctieregels binnen de selectie als geexporteerd.
CREATE OR REPLACE FUNCTION public.log_mandagen_export(
  p_project_id uuid,
  p_van date,
  p_tot date,
  p_dienstverband text,
  p_bestandsnaam text DEFAULT NULL
)
RETURNS public.mandagen_exports
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
